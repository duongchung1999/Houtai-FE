...
export default class AssignModelgPage extends Vue {
    ...
    async mounted() {
      ...
      this.users.forEach(async user => {
        const models = await ModelAPI.getList(user.id)
    ...
    showDialog(user: User) {
      ...
      const models = this.list.find(e => e.user.id == this.user.id).models
      this.userModels = []
    ...
    async assign() {
      ...
      // 更新表格数据
      const um = this.list.find(e => e.user.id == this.user.id)
