...
export class DynamicCodeAPI {
  static getByModelId(modelId: number): Promise<DynamicCode> {
    return get(`${BASE_PATH}/by-model-id/${modelId}`)
  }

  static add(code: DynamicCode): Promise<DynamicCode> {
    return add(`${BASE_PATH}`, code)
  }

  static del(codeId: number): Promise<DynamicCode> {
    return del(`${BASE_PATH}/${codeId}`)
  }
