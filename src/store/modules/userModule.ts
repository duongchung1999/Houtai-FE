...
export default class UserModule extends VuexModule {
    ...
    public async login(user: User) {
        var { token, refreshToken } = await UserAPI.login(user);
        setToken(token);
    ...
    public async getInfo(userId: number) {
        var result = await UserAPI.getInfo(userId);
        return {
            nowUser: result,
            id: result.id,
    ...
    public async update(u: User) {
        var result = await UserAPI.update(u);
    ...
    public async add(u: User) {
        var result = await UserAPI.register(u);
